...
public class ImageCacheService : IImageCacheService
{
    ...
    {
        _logger = logger;
        _cachePath = Plugin.Instance?.CacheFolderPath ?? Path.Combine(Path.GetTempPath(), "TagTurbo", "cache");
    ...
    private static void AppendImageTypeFingerprint(StringBuilder sb, Configuration.ImageTypeConfig c)
    {
        ...
        if (c.SourcePanel != null) AppendPanelFingerprint(sb, c.SourcePanel);
        if (c.FileTypePanel != null) AppendPanelFingerprint(sb, c.FileTypePanel);
    ...
    private static void AppendPanelFingerprint(StringBuilder sb, Configuration.BadgePanelSettings p)
    {
        ...
        sb.Append(string.Join(",", p.EnabledBadges ?? new List<string>()));
