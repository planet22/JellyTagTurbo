...
using Jellyfin.Plugin.TagTurbo.Configuration;
...
public class ImageOverlayService : IImageOverlayService, IDisposable
{
    ...
    private static BadgePanelSettings GetPanelForCategory(BadgeCategory category, ImageTypeConfig imageConfig)
    {
        return category switch
        {
            ...
            // Null-safe for configs saved before SourcePanel/FileTypePanel existed
            BadgeCategory.Source => imageConfig.SourcePanel ?? new Configuration.BadgePanelSettings { Enabled = false },
            BadgeCategory.FileType => imageConfig.FileTypePanel ?? new Configuration.BadgePanelSettings { Enabled = false },
    ...
    private static List<(BadgePanelSettings Panel, string Name)> GetOrderedPanels(ImageTypeConfig imageConfig)
    {
        var panels = new List<(BadgePanelSettings Panel, string Name)>
        {
            ...
            (imageConfig.SourcePanel ?? new BadgePanelSettings { Enabled = false }, "Source"),
            (imageConfig.FileTypePanel ?? new BadgePanelSettings { Enabled = false }, "FileType")
        };
    ...
    public void ReloadBadges()
    {
        ...
        try
        {
            // Do not dispose the cached bitmaps here: a render already in flight may hold an
            // undisposed reference into _rasterCache (see PrepareBadgeGroup/RenderBadgeGroup),
            // and disposing out from under it risks ObjectDisposedException or corrupted output.
            // Dropping the reference lets SkiaSharp's finalizer reclaim the native memory once
            // no longer in use; ReloadBadges only runs on rare admin actions (custom badge
            // upload/delete), so the delayed cleanup is not a meaningful cost.
            _rasterCache.Clear();
            _svgCache.Clear();
            _badgesLoaded = false;
        }
    ...
    private static (string bg, string text, byte opacity, int cornerRadius) ResolveBadgeStyle(BadgeInfo badge, BadgePanelSettings panel, ImageTypeConfig imageConfig)
    {
        ...
        if (badge.Category == BadgeCategory.Subtitle)
        {
            ...
            // VostBgOpacity/VostCornerRadius are non-nullable ints with sensible defaults, so
            // always apply them — a ">0" guard would silently ignore an intentional opacity of 0
            // (fully transparent).
            opacity = (byte)Math.Clamp(imageConfig.VostBgOpacity, 0, 255);
            cornerRadius = Math.Clamp(imageConfig.VostCornerRadius, 0, 50);
        }
