...
using Jellyfin.Plugin.TagTurbo.Services;
using static Jellyfin.Plugin.TagTurbo.Configuration.OutputImageFormat;
...
public partial class ImageOverlayMiddleware
{
    ...
    private static ImageTypeConfig ApplySizeReduction(ImageTypeConfig source, int reduction)
    {
        ...
        var clone = new ImageTypeConfig
        {
            ...
            SourcePanel = ClonePanelWithReduction(source.SourcePanel ?? new BadgePanelSettings { Enabled = false }, reduction),
            FileTypePanel = ClonePanelWithReduction(source.FileTypePanel ?? new BadgePanelSettings { Enabled = false }, reduction),
    ...
    private static BadgePanelSettings ClonePanelWithReduction(BadgePanelSettings panel, int reduction)
    {
        return new BadgePanelSettings
        {
            ...
            BadgeTypeOverrides = new List<BadgeTypeStyleOverride>(panel.BadgeTypeOverrides ?? new List<BadgeTypeStyleOverride>()),
            EnabledBadges = new List<string>(panel.EnabledBadges ?? new List<string>())
        };
    }
}
